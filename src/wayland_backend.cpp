...
static void registry_add(void*, wl_registry* reg, uint32_t name, const char* iface, uint32_t) {
    if (strcmp(iface, wl_compositor_interface.name) == 0) 
        ...
        layer_shell = (zwlr_layer_shell_v1*)wl_registry_bind(reg, name, &zwlr_layer_shell_v1_interface, 1);
}

static const wl_registry_listener reg_listener = { .global = registry_add, .global_remove = [](void*, wl_registry*, uint32_t){} };

static void layer_surface_configure(void *, struct zwlr_layer_surface_v1 *ls, uint32_t serial, uint32_t w, uint32_t h) {
...
int Wayland::init() {
    ...
    if (!compositor || !layer_shell || !shm) {
        log_msg(ERROR, "Missing required Wayland interfaces (compositor, shm, or layer_shell)");
        return 0;
    }
    ...
    layer_surface = zwlr_layer_shell_v1_get_layer_surface(
        layer_shell, surface, nullptr, ZWLR_LAYER_SHELL_V1_LAYER_BACKGROUND, "waul-wallpaper");
    ...
    if (f) {
        char buf[1024];
        if (fgets(buf, sizeof(buf), f)) current_wall = buf;
        fclose(f);
    }
...
std::string Wayland::get_current_wallpaper() {
    return current_wall;
}

void Wayland::run() {
    int ipc_sock = ipc_server_init();
    if (ipc_sock < 0) return;
    ...
    while (running) {
        ...
        if (fds[1].revents & POLLIN) {
            ...
            if (client >= 0) ipc_handle_client(client);
        }
    }
